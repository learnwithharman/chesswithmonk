...
const Openings = () => {
    ...
    return (
        <div className={`container mx-auto ${isCourseActive ? 'p-0' : 'px-4 py-8'} h-[calc(100vh-80px)] flex flex-col transition-all duration-300`}>
            <Tabs defaultValue="all" className="flex-1 flex flex-col h-full">
                ...
                <TabsContent value="famous" className={`flex-1 min-h-0 mt-0 h-full ${!isCourseActive ? 'overflow-auto' : ''}`}>
                    <FamousOpenings
                        onCourseStart={() => setIsCourseActive(true)}
                        onCourseEnd={() => setIsCourseActive(false)}
                    />
                </TabsContent>
            </Tabs>
        </div>
