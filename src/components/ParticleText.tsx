...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const sampleText = async (): Promise<void> => {
      ...
      const lineHeight = Math.ceil(resolvedSize * 1.15);
      const padding = Math.max(12, Math.ceil(resolvedSize * 0.08));
      const offscreenW = Math.ceil(maxLineWidth + padding * 2);
      const offscreenH = Math.ceil(lines.length * lineHeight + padding * 2);
      ...
      lines.forEach((lineStr, lineIdx) => {
        const lineY = padding + (lineIdx + 0.5) * lineHeight;
        offCtx.fillText(lineStr, offscreen.width / 2, lineY);
      });
  ...
  return (
    <div
      ref={containerRef}
      className={`relative block h-full min-h-[140px] md:min-h-[180px] w-full overflow-hidden touch-none ${className}`}
      style={style}
      aria-label={text}
    >
